		...
		<div class="col-md-12">
			<div class="user-action panel">
				<ul class="nav nav-tabs">
					...
					<li class="active"><a href="<?php echo U('user/followers', array('uid'=>$user_list['uid']));?>">关注</a></li>
					<li><a href="<?php echo U('user/fans',array('uid'=>$user_list['uid']));?>">粉丝</a></li>
				</ul>
				<div class="tab-content">
					<?php if(!empty($followers)): ?><ul class="media-list">
			      		<?php if(is_array($followers)): $i = 0; $__LIST__ = $followers;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$follower): $mod = ($i % 2 );++$i;?><li class="media">
							<a class="pull-left" href="<?php echo U('user/index',array('uid'=>$follower['to_uid']));?>">
								<img src="<?php echo uavatar($follower['to_uid']);?>" class="talk-avatar middle">
							</a>
							<div class="media-body">
								<h4 class="media-heading"><a href="<?php echo U('user/index',array('uid'=>$follower['to_uid']));?>"><?php echo ($follower['username']); ?></a></h4>
								<p><?php echo ($follower['intro']); ?></p>
							</div>
